...
```bash
CUDA_VISIBLE_DEVICES=0 python -m green_bit_llm.sft.finetune --model GreenBitAI/Qwen-1.5-1.8B-layer-mix-bpw-3.0 --dataset tatsu-lab/alpaca --tune-qweight-only --batch-size 1
```
...
Run the script as follows to fine-tune the quantized model using the traditional lora adapter.
```bash
